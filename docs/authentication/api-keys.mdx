...
If you're adding the `useAPIKey` option while running your app in dev mode (`npm run dev` or `next dev`), then make sure to restart your app before trying to access it with an API key.
